...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        return new Promise((resolve, reject) => {
          ...
          this.$api("blind.order.buyfee", data).then(async function (res) {
            if (that.$tools.checkResponse(res)) {
              ...
              that.openList = that.getOpenList(result.events);
              resolve(result);
            } else {
      ...
      getOpenList (events) {
        let openIndexList = events.OpenIndex;
        if(!openIndexList) return [];
        if(!openIndexList.length) {
          var index =  openIndexList.returnValues.index;
          return  [ this.nfts[index] ]
        }

        let openList = [];
        for (var i = 0;i < openIndexList.length; i++) {
          let openIndex = openIndexList[i];
          var index = openIndex.returnValues.index;
      ...
      getOpenIndex(openIndex){
        return openIndex.returnValues.index;
      },
      closed () {
        this.step = {
          approve: 0,
