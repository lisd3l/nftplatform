...
async function createMulticall(data){
  var contract = await getContract("../build/contracts/Multicall.json");
  if(contract.error) return contract;
  try{
    return await contract.new(
      {from: provider.addresses[0] });
...
async function deploy(){
  ...
  if(!data.multicall){
    console.log("create Multicall...");
    result = await createMulticall(data);
    if(result.error){
      console.log("create Multicall error", result.error);
      await utils.writeJsonFile(filename, data);
      process.exit();
    }
    console.log("create Multicall address:", result.address);
    data.multicall = {
