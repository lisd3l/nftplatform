  ...
  <div class="config-group">
    ...
    <template v-else>
      ...
      <template v-if="isPreset">
        ...
        />
        <deploy-contract
          :label="$t('config.AuctionState')"
          propKey="auctionState"
          :dataKey="dataKey"
          :editKey="editKey"
          :showData="showData"
          @setEditKey="editKey = $event"
        ...
        />
        <deploy-contract
          :label="$t('config.AuctionExchange')"
          propKey="auctionExchange"
          :dataKey="dataKey"
          :editKey="editKey"
          :showData="showData"
          @setEditKey="editKey = $event"
        ...
        switch (type) {
          ...
          case "blindBox":
            if (
              ...
              !this.editData.transferProxyForDeprecatedAddress ||
              !this.editData.erc20TransferProxyAddress ||
              !this.editData.blindStateAddress ||
              !this.editData.copyErc721Address
            ) {
              this.$notify.error({
                title: this.$t("global.fail"),
                message:
                  this.$t("config.pleaseFirstSet") +
                  "transferProxy, transferProxyForDeprecated, erc20TransferProxy, blindState, copyErc721",
              });
              return;
            }
            args = [
          ...
          case "auctionExchange":
            if (
              ...
              !this.editData.transferProxyForDeprecatedAddress ||
              !this.editData.erc20TransferProxyAddress ||
              !this.editData.auctionStateAddress
            ) {
              this.$notify.error({
                title: this.$t("global.fail"),
                message:
                  this.$t("config.pleaseFirstSet") +
                  "transferProxy, transferProxyForDeprecated, erc20TransferProxy, auctionState",
              });
              return;
            }
            args = [
