    ...
    <div class="table-container" v-if="tab != 'config'" :key="'nfts'">
      <div class="table-header">
        <div class="left">
          <span
            class="bfont mr-10"
            :class="!box.isRepetition ? 'cblue' : 'cgreen'"
          >
            {{
              !box.isRepetition
          ...
          <template v-if="!box.isRepetition">
            <span
              class="bfont mr-10"
              :class="!repeatTip.max ? 'cred' : 'cgreen'"
            >
              {{ $t("blind.box.amount") }} &le; {{ box.amount }}</span
            >
            <span
              class="bfont mr-10"
              :class="!repeatTip.typeAmount ? 'cred' : 'cgreen'"
            >
          ...
          <span
            class="bfont mr-10"
            :class="box.amount * box.nftAmount != useAmount ? 'cred' : 'cgreen'"
          >
            {{ $t("blind.box.totalAmount") }}:{{
              box.amount * box.nftAmount
            }}({{ useAmount }})
          </span>
          <span
            class="cgreen bfont mr-10"
            v-if="useAmount == box.amount * box.nftAmount"
            >{{ $t("blind.box.success") }}</span
          >
        </div>
        <div class="right">
          <el-button @click="goConfig" size="small" type="primary">
            {{ $t("blind.set.select") }}
          </el-button>
        </div>
      </div>
      ...
      >
        ...
        <el-table-column align="center" :label="$t('blind.nft.name')">
          <template #default="scope">
            <span v-if="scope.row.metadataContent?.name">
        ...
        <el-table-column align="center" :label="$t('blind.nft.property')">
          <template #default="scope">
            <div v-if="scope.row.metadataContent?.attributes">
              <div
                v-for="(attribute, i) in scope.row.metadataContent.attributes"
                :key="i"
              >
        ...
        <el-table-column
          align="center"
          :label="$t('blind.set.audioORvideo')"
          prop="metadataContent"
        >
          <template #default="scope">
            <media
              :url="scope.row.metadataContent.animation_url"
              :isPreview="true"
              v-if="scope.row.metadataContent?.animation_url"
            ></media>
          </template>
        </el-table-column>
        ...
        >
          <template #default="scope">
            ...
            <span class="cgreen bfont" v-else
              >{{ getProbability(scope.row) }}%</span
            >
          </template>
        </el-table-column>
      ...
      <div class="table-footer">
        ...
        <div class="right">
          <el-button :loading="apiLoading" @click="addData" type="primary">{{
            $t("blind.confirm")
          }}</el-button>
        </div>
      </div>
      ...
      >
        <el-form-item :label="$t('blind.nft.blindType')" prop="address">
          <el-select
            ...
            :placeholder="$t('blind.nft.pleaseSelect')"
          >
            <el-option
        ...
        <el-form-item :label="$t('blind.nft.property')">
          <div
            ...
            v-for="(property, i) in formData.properties"
            :key="i"
          >
            <div class="property left">
              <el-input
                v-model="property.key"
                :placeholder="$t('blind.property.key')"
                @input="propertyInput"
              >
              </el-input>
            </div>
            <div class="property right">
              <el-input
                ...
                :placeholder="$t('blind.property.value')"
                @input="propertyInput"
              >
              </el-input>
            </div>
          ...
          .then((res) => {
            if (this.$tool.checkResponse(res)) {
              for (var i = 0; i < res.data.list.length; i++) {
                ...
                nft.metadataContent = JSON.parse(nft.metadataContent);
