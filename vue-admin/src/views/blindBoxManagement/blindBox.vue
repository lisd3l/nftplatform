<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input
        v-model="listQuery.blindTypeName"
        clearable
        class="filter-item w-200 mr-10"
        :placeholder="$t('blind.placeholder')"
      />
      <el-select
        v-model="listQuery.sync"
        class="filter-item w-200 mr-10"
        :placeholder="$t('blind.box.pleaseSelectSync')"
        :clearable="true"
      >
        <el-option
      ...
      <el-button class="filter-item" type="primary" @click="handlerFilter"
        ><el-icon><Search /></el-icon>{{ $t("blind.select") }}</el-button
      >
      <el-button
        class="filter-item"
        type="primary"
        @click="handlerBtn('add')"
        v-permission="['POST /admin/blind/box/add']"
        ><el-icon><Plus /></el-icon>{{ $t("blind.add") }}</el-button
      >
    </div>
    ...
    >
        ...
        <template v-slot="props">
          ...
          >
            ...
            <el-form-item :label="$t('blind.box.times')">
              <span>{{
                tableTime(props.row.startTime, props.row.endTime)
              }}</span>
            </el-form-item>
      ...
      >
        <template #default="scope">
          <div
            class="bfont"
            :class="!scope.row.isRepetition ? 'cblue' : 'cgreen'"
          >
            {{
              !scope.row.isRepetition
                ? $t("blind.box.unRepeat")
                : $t("blind.box.isRepeat")
            }}
          </div>
          ...
          <div
            class="bfont"
            :class="
              scope.row.amount * scope.row.nftAmount != scope.row.usedAmount
                ? 'cred'
                : 'cgreen'
            "
          >
            {{ $t("blind.box.totalAmount") }}:
            {{ scope.row.amount * scope.row.nftAmount }}({{
      ...
      >
        <template #default="scope">
          <el-button
            type="success"
            size="small"
            @click="handlerBtn('set', scope.row)"
            v-permission="['POST /admin/blind/box/addNfts']"
            >{{ $t("blind.box.config") }}</el-button
          >
          <el-button
            type="primary"
            size="small"
            @click="handlerBtn('edit', scope.row)"
            v-permission="['POST /admin/blind/box/edit']"
            >{{ $t("blind.edit") }}</el-button
          >

          <el-button
            type="primary"
            size="small"
            @click="handlerBtn('sign', scope.row)"
            v-permission="['POST /admin/blind/box/sign']"
            >{{ $t("blind.box.sign") }}</el-button
          >
          ...
          <el-button
            ...
            v-permission="['POST /admin/blind/box/deleted']"
            >{{ $t("blind.delete") }}</el-button
          >
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog
      ...
      :width="dialogStatus === 'del' || dialogStatus === 'sync' ? '30%' : '50%'"
      @closed="callbackCloseDialog"
    >
        ...
        >
          <el-form-item :label="$t('blind.box.blindType')" prop="address">
            <el-select
              ...
              :placeholder="$t('blind.box.pleaseSelect')"
            >
              <el-option
                v-for="(blindType, index) in blindTypes"
                :key="index"
                :label="blindType.name"
                :value="blindType.address"
              >
              </el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item :label="$t('blind.box.times')" prop="times">
            <el-date-picker
              ...
              type="datetimerange"
              :range-separator="$t('blind.time.to')"
              :start-placeholder="$t('blind.time.start')"
              :end-placeholder="$t('blind.time.end')"
          ...
          <el-form-item :label="$t('blind.box.address')" prop="paytokenId">
            <el-select
              v-model="formData.paytokenId"
              :placeholder="$t('blind.box.pleaseSelect')"
            >
              <el-option
                v-for="(paytoken, index) in paytokens"
                :key="index"
      ...
      <template v-if="dialogStatus === 'sign'">
        ...
        <div class="mb-20 text-center">
          <el-button type="warning" v-if="!connected" @click="connectWallet">
            {{ $t("global.connectWallet") }}</el-button
          >
          <el-button
            :loading="apiLoading"
            type="primary"
      ...
      <template #footer>
        <div
          ...
          v-if="dialogStatus != 'set' && dialogStatus != 'sign'"
        >
  ...
  export default {
    ...
    methods: {
      ...
      init() {
        this.paytokenList();
        this.getList();
        this.$store.dispatch("blindTypes");
      },
      paytokenList() {
        ...
        this.$api("blind.box.nftsbymulti", { boxList }).then((res) => {
          if (this.$tool.checkResponse(res)) {
            ...
            for (let i = 0; i < this.list.length; i++) {
              let usedAmount = amountDict[this.list[i].id];
      ...
      async handlerConfirm() {
        ...
        switch (this.dialogStatus) {
          case "add":
            ...
            if (this.fileImage) {
              result = await this.uploadStorage(this.fileImage);
              data.cover = result;
            }
            if (this.anim1Image) {
              result = await this.uploadStorage(this.anim1Image);
              data.anim1 = result;
            }
            if (this.anim2Image) {
              result = await this.uploadStorage(this.anim2Image);
              data.anim2 = result;
            }
            this.addBox(data);
            break;
